...
class functions
{
    ...
    // public function slugify($slug_text)
    // {
    //     return mb_strtolower(preg_replace('/[ ,.@#$%^&*()_\/=]+/', '-', trim($slug_text)), 'UTF-8');
    // }
    
    public function slugify($text, $table = 'products', $field = 'slug', $key = NULL, $value = NULL)
    {
        $slug2 = strtolower($text);
        ...
        if (empty($slug)) {
            // return 'n-a';
            $slug = mb_strtolower(preg_replace('/[ ,.@#$%^&*()_\/=]+/', '-', trim($slug2)), 'UTF-8');
        }
