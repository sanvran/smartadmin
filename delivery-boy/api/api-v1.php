...
13. update_delivery_boy_is_available
-------------------------------------------
...
if (isset($_POST['get_delivery_boy_by_id'])) {
    ...
    */
    ...
    if ($num == 1) {
        ...
        $res[0]['driving_license'] = (!empty($res[0]['driving_license'])) ? DOMAIN_URL . 'upload/delivery-boy/' . $res[0]['driving_license'] : "";
        $res[0]['national_identity_card'] = (!empty($res[0]['national_identity_card'])) ? DOMAIN_URL . 'upload/delivery-boy/' . $res[0]['national_identity_card'] : "";
...
if (isset($_POST['get_orders_by_delivery_boy_id'])) {
    ...
    */
    ...
    foreach ($res as $row) {
        ...
        $tempRow['otp'] = (!empty($row['otp']) && $row['otp'] != null && isset($config['generate-otp']) && $config['generate-otp'] != 0) ? $row['otp'] : 0;
        $tempRow['name'] = $row['name'];
        $tempRow['mobile'] = $row['mobile'];
...
if (isset($_POST['update_delivery_boy_profile'])) {
    ...
    5.update_delivery_boy_profile
		...
		new_driving_license: image_file  { jpg, png, gif, jpeg }            // {optional}
		new_national_identity_card: image_file  { jpg, png, gif, jpeg }     // {optional}
    */
    $json_response = array();
    $id =  $db->escapeString(trim($fn->xss_clean($_POST['id'])));
    ...
    */
    ...
    if (!empty($res)) {
        ...
        if ($postStatus == 'delivered') {
            $res_boy = $fn->get_data($columns = ['delivery_boy_id,final_total,total,payment_method,delivery_charge'], 'id= ' . $id, 'orders');
            $delivery_boy_name = $fn->get_data($columns = ['name'], 'id= ' . $res_boy[0]['delivery_boy_id'], 'delivery_boys');
            $currency = $fn->get_data($columns = ['value'], 'variable= "currency" ', 'settings');

            if ($res_boy[0]['delivery_boy_id'] != 0) {
                $res_bonus = $fn->get_data($columns = ['bonus,name,bonus_method'], 'id= ' . $res_boy[0]['delivery_boy_id'], 'delivery_boys');

                if (strtolower($res_boy[0]['payment_method']) == 'cod') {
                    $cash_received = $res_boy[0]['total'] + $res_boy[0]['delivery_charge'];
                    $cash_received = ceil($cash_received);
                    
                    $sql = "UPDATE delivery_boys SET cash_received = cash_received + $cash_received WHERE id=" . $res_boy[0]['delivery_boy_id'];
                    $db->sql($sql);
                    $fn->add_transaction($id, $res_boy[0]['delivery_boy_id'], 'delivery_boy_cash', $cash_received, 'Delivery boy collected COD');
                }


                if ($res_bonus[0]['bonus_method'] == "percentage") {
                    $reward = $res_boy[0]['total'] / 100 * $res_bonus[0]['bonus'];
                } else if ($res_bonus[0]['bonus_method'] == "rupees") {
...
if ((isset($_POST['update_delivery_boy_is_available'])) && ($_POST['update_delivery_boy_is_available'] == 1)) {

    /*
    13.update_delivery_boy_is_available
        accesskey:90336
        update_delivery_boy_is_available:1
        delivery_boy_id:40
        is_available:0 OR 1
    */
    ...
    if (empty($_POST['delivery_boy_id'])) {
        $response['error'] = true;
        $response['message'] = "Please pass delivery_boy_id!";
        print_r(json_encode($response));
        return false;
    }

    $delivery_boy_id = $db->escapeString(trim($fn->xss_clean($_POST['delivery_boy_id'])));
    $is_available = $db->escapeString(trim($fn->xss_clean($_POST['is_available'])));

    $sql1 = "update delivery_boys set `is_available` ='$is_available' where id = '" . $delivery_boy_id . "'";
    if ($db->sql($sql1)) {
        ...
        $response['error'] = false;
        $response['message'] = "Delivery Boy Updeted successfully.";
        $response['data'] = $res;
    } else {
        $response['error'] = true;
        $response['message'] = "Can not update delivery boy.";
    }
    print_r(json_encode($response));
}
