...
if (isset($_POST['update_order_status']) && isset($_POST['id'])) {
    ...
    if (!empty($res)) {
        ...
        if ($postStatus == 'cancelled' || $postStatus == 'returned') {
            ...
            $sql = "select o.*,obt.status as attachment_status from orders o LEFT JOIN order_bank_transfers obt ON o.id = obt.order_id where o.id=" . $id;
            $db->sql($sql);
            $res_current_status = $db->getResult();
            if (strtolower($res[0]['payment_method']) != 'cod') {
                /* update user's wallet */
                if ($res[0]['payment_method'] == "bank transfer") {
                    if ($res_current_status[0]['attachment_status'] == '0' || $res_current_status[0]['attachment_status'] == '2' ||  $res_current_status[0]['attachment_status'] == '') {
                        $rechrge_wallet = false;
                    } else {
                        $rechrge_wallet = true;
                    }
                } else {
        ...
        if ($postStatus == 'delivered') {
            $res_boy = $function->get_data($columns = ['delivery_boy_id,final_total,total,payment_method,delivery_charge'], 'id= ' . $id, 'orders');
            $currency = $function->get_data($columns = ['value'], 'variable= "currency" ', 'settings');

            if ($res_boy[0]['delivery_boy_id'] != 0) {

                $deli_boy_id = $res_boy[0]['delivery_boy_id'];
                $res_bonus = $function->get_data($columns = ['bonus,name,bonus_method'], 'id= ' . $deli_boy_id, 'delivery_boys');
                ...
                if (strtolower($res_boy[0]['payment_method']) == 'cod') {
                    ...
                    $db->sql($sql);
                    $function->add_transaction($id, $res_boy[0]['delivery_boy_id'], 'delivery_boy_cash', $cash_received, 'Delivery boy collected COD');
                }
                ...
                if ($reward > 0) {
                    ...
                    $db->sql($sql);
                    $comission = $function->add_delivery_boy_commission($res_boy[0]['delivery_boy_id'], 'credit', $reward, 'Order Delivery Commission.');
