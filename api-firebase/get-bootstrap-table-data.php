...
if (isset($_GET['table']) && $_GET['table'] == 'products') {
    ...
    if (isset($_GET['low_stock']) && $_GET['low_stock'] == 1) {
        $where .= empty($where) ? " WHERE pv.stock < $low_stock_limit AND pv.serve_for = 'Available'" : " AND pv.stock < $low_stock_limit AND pv.serve_for = 'Available'";
    }
...
// data of 'PRODUCTS' table goes here
if (isset($_GET['table']) && $_GET['table'] == 'product_variants') {
    ...
    $order = (isset($_GET['order']) && !empty($_GET['order'])) ? $db->escapeString($fn->xss_clean($_GET['order'])) : 'ASC';

    if (isset($_GET['search']) and $_GET['search'] != '') {
        $search = $db->escapeString($fn->xss_clean($_GET['search']));
        if (!empty($where)) {
            $where = " AND (pv.`product_id` like '%" . $search . "%' OR pv.`id` like '%" . $search . "%' OR p.`name` like '%" . $search . "%' OR pv.`measurement` like '%" . $search . "%' OR u.`short_code` like '%" . $search . "%' )";
        } else {
            $where = " WHERE (pv.`product_id` like '%" . $search . "%' OR pv.`id` like '%" . $search . "%' OR p.`name` like '%" . $search . "%' OR pv.`measurement` like '%" . $search . "%' OR u.`short_code` like '%" . $search . "%' )";
        }
    }

    if (isset($_GET['id']) and $_GET['id'] != '') {
        $id = $db->escapeString($fn->xss_clean($_GET['id']));
        $where .= (!empty($where)) ? " AND pv.product_id =" . $id : " WHERE pv.product_id = " . $id;
    }
    ...
    $join = "LEFT JOIN `products` p ON p.id = pv.product_id
            LEFT JOIN `unit` u ON u.id = pv.measurement_unit_id 
            LEFT JOIN `flash_sales_products` fp ON fp.product_id = pv.product_id ";

    $sql = "SELECT COUNT(pv.id) as `total` FROM `product_variant` pv $join " . $where . "";
    $db->sql($sql);
    $res = $db->getResult();
    ...
    $sql = "SELECT pv.*,p.name,p.status,p.tax_id,p.image, p.indicator, p.manufacturer, p.made_in, p.return_status, p.cancelable_status, p.till_status,p.description, p.id as product_id, u.short_code ,fp.price as sales_price,fp.discounted_price as sales_discounted_price,fp.status as sales_status,fp.start_date,fp.end_date
            FROM `product_variant` pv $join $where ORDER BY $sort $order LIMIT $offset, $limit";
    $db->sql($sql);
    $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        if (!empty($row['stock_unit_id'])) {
            $stock_unit = $fn->get_data($columns = ['short_code as stock_unit'], 'id=' . $row['stock_unit_id'], 'unit');
            $tempRow['stock'] = $row['stock'] . ' ' . $stock_unit[0]['stock_unit'];
        }

        $operate = '<a class="btn btn-xs btn-info" href="product-detail.php?id=' . $row['id'] . '" title="View"><i class="fa fa-folder-open"></i></a>&nbsp;';
        $operate .= '<a class="btn btn-xs btn-primary" href="edit-product.php?id=' . $row['product_id'] . '" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;';
        ...
        $tempRow['till_status'] = $row['cancelable_status'] == 1 ? $till_status : "<label class='label label-info'>Not Applicable</label>";
        $tempRow['serve_for'] = $row['serve_for'] == 'Sold Out' ? "<span class='label label-danger'>Sold Out</label>" : "<span class='label label-success'>Available</label>";
        $tempRow['status'] = ($row['status'] == 1) ? "<span class='label label-success'>Active</label>" : "<span class='label label-danger'>De-active</label>";
...
if (isset($_GET['table']) && $_GET['table'] == 'slider') {
    ...
    if (isset($_GET['search'])) {
        $search = $db->escapeString($fn->xss_clean($_GET['search']));
        $where = " Where `id` like '%" . $search . "%' OR `title` like '%" . $search . "%' OR `type` like '%" . $search . "%' OR `short_description` like '%" . $search . "%' ";
    }
    ...
    foreach ($res as $row) {
        if ($row['status'] == 1) {
            $operate = "<a class='btn btn-xs btn-warning set-slider-deactive' data-id='" . $row['id'] . "' title='Hide'>  <i class='fa fa-eye'></i></a>&nbsp;";
        } elseif ($row['status'] == 0) {
            $operate = "<a class='btn btn-xs btn-success set-slider-active' data-id='" . $row['id'] . "' title='Show'>  <i class='fa fa-eye-slash'></i></a>&nbsp;";
        }
...
if (isset($_GET['table']) && $_GET['table'] == 'offers') {
    ...
    if (isset($_GET['search'])) {
        $search = $db->escapeString($fn->xss_clean($_GET['search']));
        $where = " Where `id` like '%" . $search . "%' OR `type` like '%" . $search . "%' OR `position` like '%" . $search . "%' ";
    }
    ...
    $rows = array();
    $tempRow = array();
    $images  = DOMAIN_URL . 'images/video_images.png';
    foreach ($res as $row) {
        if ($row['status'] == 1) {
            $operate = "<a class='btn btn-xs btn-warning set-offers-deactive' data-id='" . $row['id'] . "' title='Hide'>  <i class='fa fa-eye'></i></a>&nbsp;";
        } elseif ($row['status'] == 0) {
            $operate = "<a class='btn btn-xs btn-success set-offers-active' data-id='" . $row['id'] . "' title='Show'>  <i class='fa fa-eye-slash'></i></a>&nbsp;";
        }
        $operate .= " <a class='btn btn-xs btn-danger delete-offer' data-id='" . $row['id'] . "' data-image='" . $row['image'] . "' title='Delete'><i class='fa fa-trash-o'></i></a>&nbsp;";

        $tempRow['id'] = $row['id'];
        $tempRow['type'] = $row['type'];
        $tempRow['image'] = (!empty($row['image']) && $row['type'] == 'image') ? "<a data-lightbox='offer' href='" . $row['image'] . "'><img src='" . $row['image'] . "' width='40'/></a>" : "<a data-lightbox='offer' href='"  . $images . "'><img src='" . $images . "' width='40'/></a>";
...
if (isset($_GET['table']) && $_GET['table'] == 'media') {
    ...
    foreach ($res as $row) {
        ...
        $tempRow['size'] = ($row['size'] > 1) ? $fn->formatBytes($row['size']) : $row['size'];
...
if (isset($_GET['table']) && $_GET['table'] == 'promo-codes') {
    ...
    foreach ($res as $row) {
        ...
        $operate = "<a class='btn btn-xs btn-primary edit-promo-code' data-id='" . $row['id'] . "' data-toggle='modal' data-backdrop='static' data-keyboard='false' data-target='#editPromoCodeModal' title='Edit'><i class='fa fa-pencil-square-o'></i></a>";
...
// data of 'wishlists' table goes here
if (isset($_GET['table']) && $_GET['table'] == 'wishlists') {
    $where = '';
    $offset = (isset($_GET['offset']) && !empty($_GET['offset']) && is_numeric($_GET['offset'])) ? $db->escapeString($_GET['offset']) : 0;
    $limit = (isset($_GET['limit']) && !empty($_GET['limit']) && is_numeric($_GET['limit'])) ? $db->escapeString($_GET['limit']) : 10;
    $sort = (isset($_GET['sort']) && !empty($_GET['sort'])) ? $db->escapeString($_GET['sort']) : 'total_qty';
    $order = (isset($_GET['order']) && !empty($_GET['order'])) ? $db->escapeString($_GET['order']) : 'DESC';

    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $search = $db->escapeString($fn->xss_clean($_GET['search']));
        $where = " AND f.`id` like '%$search%' OR u.`name` LIKE '%$search%' OR p.`name` LIKE '%$search%'";
    }

    $sql = "SELECT f.*,count(f.id) as total_qty,u.name as user_name ,p.name as product_name FROM `favorites` f JOIN users u ON u.id= f.user_id JOIN products p ON p.id=f.product_id where f.date_created > DATE_SUB(NOW(), INTERVAL 1 MONTH) " . $where . " GROUP BY f.product_id ORDER by total_qty DESC";
    $db->sql($sql);
    $res = $db->getResult();
    $total = $db->numRows($res);

    $sql = "SELECT f.*,count(f.id) as total_qty,u.name as user_name ,p.name as product_name FROM `favorites` f JOIN users u ON u.id= f.user_id JOIN products p ON p.id=f.product_id where f.date_created > DATE_SUB(NOW(), INTERVAL 1 MONTH) " . $where . " GROUP BY f.product_id ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $operate = ' <a href="view-product-variants.php?id=' . $row['product_id'] . '"><i class="fa fa-eye"></i>View</a>';
...
if (isset($_GET['table']) && $_GET['table'] == 'delivery-boy-cash') {
    ...
    $offset = (isset($_GET['offset']) && !empty($_GET['offset']) && is_numeric($_GET['offset'])) ? $db->escapeString($_GET['offset']) : 0;
    $limit = (isset($_GET['limit']) && !empty($_GET['limit']) && is_numeric($_GET['limit'])) ? $db->escapeString($_GET['limit']) : 10;
    $sort = (isset($_GET['sort']) && !empty($_GET['sort'])) ? $db->escapeString($_GET['sort']) : 't.id';
    $order = (isset($_GET['order']) && !empty($_GET['order'])) ? $db->escapeString($_GET['order']) : 'DESC';
    $where = " where (t.type='delivery_boy_cash'||t.type='delivery_boy_cash_collection')";

    $where .= (isset($_GET['type']) && !empty($_GET['type'])) ? " AND (t.type = '" . $_GET['type'] . "')" : '';
    $where .= (isset($_GET['delivery_boy_id']) && !empty($_GET['delivery_boy_id'])) ? " AND t.user_id = " . $_GET['delivery_boy_id'] : '';
    $where .= (isset($_GET['date']) && !empty($_GET['date'])) ? " AND DATE_FORMAT(t.transaction_date, '%Y-%m-%d')='" . $_GET['date'] . "' " : '';
    ...
    foreach ($res as $row) {
        ...
        $tempRow['address'] = $row['address'];
        $tempRow['delivery_boy_id'] = $row['user_id'];
        $tempRow['amount'] = $row['amount'];
        $tempRow['type'] = $type;
...
if (isset($_GET['table']) && $_GET['table'] == 'ticket_types') {
    ...
    foreach ($res as $row) {
        $operate = "<a class='btn btn-xs btn-primary edit-ticket-types' data-id='" . $row['id'] . "' data-toggle='modal' data-backdrop='static' data-keyboard='false' data-target='#editTicketTypes' title='Edit'><i class='fa fa-edit'></i></a>";
        $operate .= " <a class='btn btn-xs btn-danger delete-ticket-types' data-id='" . $row['id'] . "' title='Delete'><i class='fa fa-trash-o'></i></a>";
...
if (isset($_GET['table']) && $_GET['table'] == 'ticket') {

    $offset = (isset($_GET['offset']) && !empty($_GET['offset']) && is_numeric($_GET['offset'])) ? $db->escapeString($_GET['offset']) : 0;
    $limit = (isset($_GET['limit']) && !empty($_GET['limit']) && is_numeric($_GET['limit'])) ? $db->escapeString($_GET['limit']) : 10;
    $sort = (isset($_GET['sort']) && !empty($_GET['sort'])) ? $db->escapeString($_GET['sort']) : 'id';
    $order = (isset($_GET['order']) && !empty($_GET['order'])) ? $db->escapeString($_GET['order']) : 'ASC';
    $where = '';

    if (isset($_GET['search']) and $_GET['search'] != '') {
        $search = $db->escapeString($fn->xss_clean($_GET['search']));
        $where = " where (t.`id` like '%" . $search . "%' OR t.`title` like '%" . $search . "%' OR t.`message` like '%" . $search . "%' OR t.`email` like '%" . $search . "%' OR t.`status` like '%" . $search . "%' )";
    }

    $sql = "SELECT COUNT(t.`id`) as total FROM `tickets` t LEFT JOIN ticket_type ti_t ON t.type_id = ti_t.id LEFT JOIN users u ON t.user_id = u.id" . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];

    $sql = "SELECT t.*,ti_t.type,u.name FROM `tickets` t LEFT JOIN ticket_type ti_t ON t.type_id = ti_t.id LEFT JOIN users u ON t.user_id = u.id" . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $operate = "<a class='btn btn-xs btn-primary edit-ticket' data-id='" . $row['id'] . "' data-toggle='modal' data-target='#editTicketModal' title='Edit'><i class='fa fa-edit'></i></a>&nbsp;";
        $operate .= "<a class='btn btn-xs btn-success view-ticket' href='support-view.php?id=" . $row['id'] . " ' title='View'><i class='fa fa-commenting'></i></a>&nbsp;";
        $operate .= "<a class='btn btn-xs btn-danger' href='delete-complaints.php?id=" . $row['id'] . " ' title='Delete'><i class='fa fa-trash-o'></i></a>";

        $tempRow['id'] = $row['id'];
        $tempRow['ticket_type_id'] = $row['type_id'];
        $tempRow['user_id'] = $row['user_id'];
        $tempRow['ticket_type'] = $row['type'];
